 ...
 * own `newspack_cache_cozy_minute` recurrence (so scheduling doesn't depend on any
 * other plugin's cron interval being loaded).
 */
class Cache_Cozy {

	/** Option holding the loopback secret. */
	private const SECRET_OPTION = 'newspack_cache_cozy_secret';

	/** Option holding the loopback's HTTP Basic credential, encrypted at rest. */
	public const AUTH_OPTION = 'newspack_cache_cozy_auth';
	...
	public const CRON_HOOK = 'newspack_cache_cozy_tick';

	/** Self-owned 60s recurrence so scheduling never depends on another plugin. */
	public const CRON_SCHEDULE = 'newspack_cache_cozy_minute';
...
if ( ! \defined( 'NEWSPACK_CACHE_COZY_SKIP_BOOT' ) ) {
	Cache_Cozy::register();
}
