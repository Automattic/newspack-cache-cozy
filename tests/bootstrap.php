<?php
/**
 * PHPUnit bootstrap for Newspack Cache Cozy tests.
 *
...
// cache-cozy uses no substrate config in tests (the tick node only needs Core /
// Message / Timer_Node and a Router); the substrate falls back to its bundled
// newspack-nodes-config.php defaults. No LOCAL_NEWSPACK_NODES_CONF override.
...
// clone (the substrate must be checked out alongside this plugin to test).
require_once \dirname( __DIR__, 2 ) . '/newspack-nodes/newspack-nodes.php';
// Substrate test helpers: the TestCase base (resets Core in setUp) + the
// Capture_Sink_Node double the tick test asserts emitted messages against.
...
// This plugin's own classmap (includes/) — Cache_Cozy_Tick_Node.
require_once \dirname( __DIR__ ) . '/vendor/autoload.php';

// The warmer is a self-contained mu-plugin drop-in (not autoloaded). Load its
// classes for the unit tests; SKIP_BOOT suppresses the file's register()
// side effect so requiring it doesn't install the cold-cache hooks.
\define( 'NEWSPACK_CACHE_COZY_SKIP_BOOT', true );
require_once \dirname( __DIR__ ) . '/mu-plugins/01-newspack-cache-cozy.php';
